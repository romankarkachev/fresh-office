...
/**
 * This is the model class for table "payment_orders_files".
 *
 * @property integer $id
 ...
 *
 * @property PaymentOrders $po
 * @property User $uploadedBy
 */
class PaymentOrdersFiles extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function rules()
    {
        return [
            ...
            [['po_id'], 'exist', 'skipOnError' => true, 'targetClass' => PaymentOrders::className(), 'targetAttribute' => ['po_id' => 'id']],
    ...
    {
        $filepath = Yii::getAlias('@uploads-payment-orders-fs');
        if (!empty($obj_id)) {
            ...
            $object = PaymentOrders::findOne($obj_id);
            if ($object) {
                // например: \uploads\payment-orders\1970\01\01\1524
    ...
    {
        return $this->hasOne(PaymentOrders::className(), ['id' => 'po_id']);
    }
