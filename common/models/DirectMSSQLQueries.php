...
class DirectMSSQLQueries extends Model
{
    ...
    const PROJECT_STATES_FOR_LOGIST_FILTER = [
        ...
        ProjectsStates::STATE_ОДОБРЕНО_ПРОИЗВОДСТВОМ, // 14
...
SELECT
    ...
    LTRIM(RTRIM(LIST_EMAIL_CLIENT.EMAILS)) AS emails
...
LEFT JOIN (
	SELECT ID_CONTACT_MAN, STUFF((SELECT \', \' + LTRIM(RTRIM(email)) FROM LIST_EMAIL_CLIENT LT WHERE LT.ID_CONTACT_MAN = LIST_EMAIL_CLIENT.ID_CONTACT_MAN FOR XML PATH(\'\')), 1, 1, \'\') AS EMAILS
	FROM LIST_EMAIL_CLIENT
	GROUP BY ID_CONTACT_MAN
) AS LIST_EMAIL_CLIENT ON LIST_EMAIL_CLIENT.ID_CONTACT_MAN = LIST_CONTACT_MAN.ID_CONTACT_MAN
WHERE
